          ...
          {obligations.some(o => o.status === 'overdue' || o.status === 'rejected') && (
            <div className="space-y-2">
              {obligations.filter(o => o.status === 'overdue').map(o => (
                <div key={o.id} className="flex items-center gap-3 rounded-xl border border-destructive/20 bg-destructive/5 p-3">
                  <AlertTriangle className="h-4 w-4 text-destructive shrink-0" />
                  <p className="flex-1 text-sm text-foreground"><strong>{o.concept}</strong> — {o.amount} overdue since {o.dueDate}</p>
                  <button className="text-xs font-medium text-primary hover:underline">Pay Now</button>
                </div>
              ))}
              {obligations.filter(o => o.status === 'rejected').map(o => (
                <div key={o.id} className="flex items-center gap-3 rounded-xl border border-destructive/20 bg-destructive/5 p-3">
                  ...
                  <button className="text-xs font-medium text-primary hover:underline">Retry</button>
                </div>
              ))}
            </div>
          )}
          ...
          {receipts.length > 0 ? (
            receipts.map(o => (
              <div key={o.id} className="flex items-center gap-4 rounded-xl border border-border bg-card px-4 py-3.5">
                ...
                <button className="flex items-center gap-1 rounded-lg bg-muted px-2.5 py-1.5 text-xs font-medium text-foreground hover:bg-muted/80 transition-colors">
